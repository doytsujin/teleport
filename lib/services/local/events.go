...
type eventTranslator interface {
	// translate translates a matching backend event into a list of higher level
	// events.
...
// 	// parseResource parses resource from the backend event.
// 	translateMatching(event backend.Event) ([]services.Event, error)
// }

// func translateWithResourceParser(parser resourceParser, event backend.Event) ([]services.Event, error) {
// 	resource, err := parser.parseResource(event)
// 	if err != nil {
// 		return nil, trace.Wrap(err)
// 	}
...
func (p *clusterConfigParser) translateMatching(event backend.Event) ([]services.Event, error) {
	...
	case backend.OpPut:
		// For backward compatibility reasons, do not use the ClusterConfig
...
func (p *clusterNetworkingConfigParser) translateMatching(event backend.Event) ([]services.Event, error) {
	...
	case backend.OpPut:
		...
		// For backward compatibility reasons, also emit an event indicating
