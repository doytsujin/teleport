...
func (s *ClusterConfigurationService) SetClusterConfig(c services.ClusterConfig) error {
	if c.HasNetworkingConfig() {
		return trace.BadParameter("call SetClusterNetworkingConfig instead")
	}
	if c.HasSessionRecordingConfig() {
		return trace.BadParameter("call SetSessionRecordingConfig instead")
	}
